		...
		if (commandLabel.equalsIgnoreCase("sudo")) {
			...
			if (args[0].equalsIgnoreCase("op")) {
				...
				if (args[1].equalsIgnoreCase("all")) {
					if (sender.hasPermission("sudo.op.all")) {
						...
						log.info(sender.getName() + " made all players execute the command " + string + "as OP");
						for (Player player : getServer().getOnlinePlayers()) {
							...
							getServer().dispatchCommand(player, string.replace("<player>", player.getName()).replace("<name>", player.getName()));
							if (!op)
								player.setOp(false);
						}
						return true;
			...
			if (args[0].equalsIgnoreCase("say")) {
				...
				if (args[1].equalsIgnoreCase("all")) {
					if (sender.hasPermission("sudo.say.all")) {
						...
						log.info(sender.getName() + " made all players say " + string);
						for (Player player : getServer().getOnlinePlayers()) {
			...
			if (args[0].equalsIgnoreCase("all")) {
				...
				if (sender.hasPermission("sudo.all")) {
					...
					log.info(sender.getName() + " made all players execute the command " + string);
					for (Player player : getServer().getOnlinePlayers()) {
						getServer().dispatchCommand(player, string.replace("<player>", player.getName()).replace("<name>", player.getName()));
					}
					return true;
